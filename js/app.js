...
// Komponen Icon yang diperbaiki untuk struktur Lucide CDN
const Icon = ({ icon, size = 20, className = "" }) => {
    // Cek apakah icon valid dan memiliki data children (index ke-2 pada Lucide UMD)
    if (!icon || !Array.isArray(icon) || !icon[2]) return null;
...
// ... (ProgressBar, SimpleLineChart, Card tetap sama seperti sebelumnya)
    ...
    return (
        <div className={`min-h-screen transition-all ${darkMode ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
            ...
            <main className="max-w-5xl mx-auto p-6 animate-in">
                <div className={`p-10 rounded-[2.5rem] border ${darkMode ? 'bg-slate-900 border-slate-800 shadow-2xl' : 'bg-white border-slate-200 shadow-sm'}`}>
                    <h2 className="text-4xl font-black mb-6 tracking-tight">Dashboard Akademik</h2>
